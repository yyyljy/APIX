...
sequenceDiagram
    ...
    alt Payment Valid
        Gateway->>Gateway: Verify: (Status==1 or PENDING) && (To==Seller) && (Value>=Price)
...
*   **Retry Logic:** If the receipt is not found immediatey, implement exponential backoff retries (e.g., 3 attempts).
