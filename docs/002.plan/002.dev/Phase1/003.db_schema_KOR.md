...
erDiagram
    ...
    LISTINGS {
        ...
        varchar origin_auth_header "인증 헤더 키 (예: Authorization)"
        text origin_auth_value "인증 값 (암호화됨)"
        boolean is_active
    }
...
| `origin_auth_header` | VARCHAR(100) | DEFAULT 'Authorization' | 업스트림 인증을 위한 헤더 키. |
| `origin_auth_value` | TEXT | NULL | 업스트림 인증을 위한 암호화된 자격 증명. |
...
| `amount_paid` | DECIMAL(78, 18) | NULL | 온체인에서 확인된 실제 금액 (Wei). |
| `memo` | TEXT | NULL | 선택적 결제 메모 또는 메타데이터. |
