...
erDiagram
    ...
    LISTINGS {
        ...
        varchar origin_auth_header "Auth Header Key (e.g. Authorization)"
        text origin_auth_value "Auth Value (Encrypted)"
        boolean is_active
    }
...
| `origin_auth_header` | VARCHAR(100) | DEFAULT 'Authorization' | Header key for upstream auth. |
| `origin_auth_value` | TEXT | NULL | Encrypted credential for upstream auth. |
...
| `amount_paid` | DECIMAL(78, 18) | NULL | Actual value verified on-chain (Wei). |
| `memo` | TEXT | NULL | Optional payment note or metadata. |
...
CREATE TABLE IF NOT EXISTS listings (
    ...
    origin_auth_header VARCHAR(100) DEFAULT 'Authorization',
    origin_auth_value TEXT,
    description TEXT,
    categories TEXT[],
