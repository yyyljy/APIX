...
`apix-cloud`를 경유하지 않는 경량 경로로 재구성한다.

- **전제**: API 공급자(판매자)의 백엔드 서버는 존재하며, SDK는 그 백엔드에 내장/연동되어 동작한다.
- SDK 개발자 관점에서는 “백엔드는 있고 SDK가 그 안에서 Cloud 없이 동작”이 목표가 맞다.

- 기존: 클라이언트 → 백엔드 → SDK → Apix Cloud → L1
...
- 핵심 정합성: 요청 라우팅/커밋/롤백은 백엔드가 담당하고, `verifyPayment`는 SDK가 Cloud 또는 L1 모드로 수행한다.
- `useCloudVerification=false`면 Apix Cloud HTTP endpoint 없이 `APIX_RPC_URL` 기반 L1 검증 루트로 운영 가능하다.

- SDK가 현재 `/v1/verify`, `/v1/session/*`를 cloud로 호출하고 있음: `apix-sdk-node/index.js:297`, `apix-sdk-node/index.js:199`
- cloud에서 실제 tx 검증(`tx_hash`, recipient, 금액, chain, confirmation)을 수행하고 있음: `apix-cloud/main.go:742`, `apix-cloud/main.go:297`, `apix-cloud/main.go:528`
...
4. 백엔드 SDK가 tx hash를 L1 기준으로 검증(Cloud 대체)
...
- `verifyPayment`에서 모드 분기 도입
  - 현재: `axios.post(${facilitatorUrl}/v1/verify)` 경로 사용(기본 동작, `useCloudVerification=true`)
  - 추가: `useCloudVerification=false`일 때 `APIX_RPC_URL` 기반 직접 검증 경로 사용
- 직접 검증 구성 옵션
  - `rpcUrl` (`APIX_RPC_URL`)
...
- 세션 관리 의존성 제거/내부화
  - 현재: `/v1/session/validate|start|commit|rollback` 호출 경로 존재
  - 변경: local store 기반 인터페이스로 대체(기본은 in-memory/file)
- JWT 발급/검증은 SDK가 직접 담당
- 옵션으로 cloud 모드 하위호환 유지

### 5.2 백엔드(`demo/backend`) 변경

- 기존 미들웨어 진입점은 유지 (`/apix-product`)
- 402 반환/세션 시작/커밋/롤백 호출은 유지
- `ApixMiddleware` 설정에 결제 검증 모드 플래그 추가
  - `useCloudVerification`
  - `rpcUrl`, `rpcTimeoutMs`, `rpcMaxRetries`, `defaultMinConfirmations`
- `APIX_USE_CLOUD_VERIFICATION`로 SDK가 L1 직접 검증 모드 사용 여부 제어
- `APIX_USE_CLOUD_SESSION_STATE`가 `false`일 때도 다중 인스턴스 안전하게 동작하도록 스토어 설정을 명확화
...
### Phase 1: Cloud Adapter 추가
- 현재 동작 유지 하에 SDK에 `Verifier`와 `SessionStore` 인터페이스 도입
- cloud 사용/로컬 사용 feature flag 분리
...
### Phase 3: Cloud 제거
- 운영 배포 경로에서 cloud 호출 비활성화
- 기존 cloud 기능은 참고 모듈로 축소
...
요청하신 “클라이언트-SDK-L1만”은 코드적으로는 **가능**하지만, 단순 삭제가 아닌 SDK 내 상태/검증 책임 이관이 선행되어야 한다.

즉, `apix-cloud`를 없앤다는 뜻은 단순히 URL을 비우는 작업이 아니라, SDK가 내장된 판매자 백엔드 내부에서:
